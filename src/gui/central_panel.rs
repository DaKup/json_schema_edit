    ...
    match &mut value {
        Value::Object(object_value) => {
            object_value.iter_mut().for_each(|(key, value)| {
                ...
                deep_hash_map(ctx, frame, ui, key, value, path);
            });
        }
        Value::String(string_value) => {
            if key == "description" {
                ui.label("");
                ui.label(path.join(" -> ").add(":"));

                ui.add(
                    egui::TextEdit::multiline(string_value).code_editor(), // .desired_rows(100)
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    app.state.json_hash_map.iter_mut().for_each(|(key, value)| {
                        deep_hash_map(ctx, frame, ui, key, value, vec!["root"]);
                    });
                })
        });
    });
